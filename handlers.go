...
func getSetiPublicConnection() swift.Connection {
  ...
  if err != nil {
    ...
    c = swift.Connection{
      ...
      AuthUrl: os.Getenv("SWIFT_AUTH_URL") + "/v3",  //make sure to append "/v3"
      Domain: os.Getenv("SWIFT_API_DOMAIN"), //domainName
      DomainId: os.Getenv("SWIFT_API_DOMAIN_ID"), //domainId
      Tenant: os.Getenv("SWIFT_TENANT"), //project in vcap_services on bluemix
      TenantId: os.Getenv("SWIFT_TENANT_ID"), //projectId in vcap_services on bluemix
    }
...
func GetACARawDataTempURL (w http.ResponseWriter, r *http.Request) {
  ...
  if swift_secret_key == "" {
    ReturnError(w, 500, "temp_url_error", "credentials not found")
    return
  }
  ...
  if os.Getenv("EXPIRATION_TIME") != "" {
    fmt.Println("expiration: " + os.Getenv("EXPIRATION_TIME"))
    extim, _ := time.ParseDuration(os.Getenv("EXPIRATION_TIME"))
...
func GetACARawData (w http.ResponseWriter, r *http.Request) {
  ...
  err := c.Authenticate()
  if err != nil {
      ReturnError(w, 500, "aca_data_error", err.Error())
      return
  }
